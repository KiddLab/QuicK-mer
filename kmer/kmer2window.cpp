...
//  Add direct depth output in window function 11/9/2016
...
int main(int argc, char * argv[])
{
	...
	if (files != 4)
	{
		...
		printf("-D for calculating depth in window without copy number normalization.\n");
		return 1;
	}
	//Read in window file
	...
	while (window_file >> chr >> bin_start >> bin_end >> bin_count)
	{
		...
		if (use_depth == 0)
		{
			...
			for (int i = 0; i < bin_count; i++) bool_sum += control_region[i];
		}
		...
		if (use_depth)
		{
			std::cout << chr <<'\t'<< bin_start <<'\t'<< bin_end;
			std::cout << std::fixed;
			std::cout << std::setprecision(3) <<'\t'<< ave << std::endl;
			//Directly skip to next cycle
			continue;
		}
